...
// const searchToHTML = results => {
//   return `<div class='song-list'>${results}</div>`;
// }
...
const render = (playlist) => {
  ...
  songList.innerHTML = combinedHTML;
}
render(playlist);

for (i = 0; i < playlist.songs.length; i++) {
  let song = playlist.songs[i].name;
  console.log(song)
  ...
  search.addEventListener('input', e => {
    ...
    if (song.toLowerCase().includes(input.toLowerCase())) {
      results.push(song);
      console.log(results);
      // results += ObjectToHTML;
